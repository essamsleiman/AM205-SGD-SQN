...
def plot_convergence(losses, label, fig, ax, color):
    ...
    ax.set_ylabel(r"$\lambda_{\mathrm{min}}$")
    ax.grid()
    ax.legend()
    fig.tight_layout()
    fig.savefig("Convex_Min_EigVals.png", dpi=350)
    ...
    def stepML2(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # get the Hessian approximation
                Hessian = state["Hessian"]
                min_hessian_list.append(np.min(np.linalg.eigvals(Hessian)))
        ...
        return loss, new_theta, min_hessian_list
...
def optimize_ML_SQN(X, y, thetas, epochs):
    ...
    for epoch in range(epochs):
        ...
        for j in range(0, n_samples, args.batch_size):
            ...
            if args.model == "ML1":
                ...
                loss ,new_theta, min_eig_list = optimizer.stepML2(closure=closure)

            X_total = torch.tensor(X, dtype=torch.float32).squeeze(-1)
            y_total = torch.tensor(y, dtype=torch.float32).unsqueeze(1)
            # print("criterion(model(X_total), y): ", criterion(model(X_total), y))
            total_losses.append(criterion(model(X_total), y_total).item())
            thetas.append(np.array(new_theta))
            min_eig_list_total.extend(min_eig_list)
    ...
    return total_losses, min_eig_list_total
...
if __name__ == "__main__":
    ...
    fig_converge, ax_converge = plt.subplots(figsize=(10, 4))
    for ite, optimizer in enumerate(["SQN", "SQN"]):
        lr = lr_list[ite]
        ...
        if args.model == "StrongConvex":
            ...
            if args.optimizer == "SQN":
                losses, min_eig_list_total = optimize_ML_SQN(A_batch, b_batch, thetas, args.epochs)
                thetas = np.array(thetas)
            else:
        ...
        if args.model == "StrongConvex":
            ...
            plot_convergence(min_eig_list_total, labels[ite], fig_converge, ax_converge, colors[ite])
